...
CREATE TABLE IF NOT EXISTS sites (
  id text PRIMARY KEY,
  name text NOT NULL,
  address_context text NOT NULL DEFAULT '',
  created_at timestamptz NOT NULL
);
...
CREATE TABLE IF NOT EXISTS device_identifiers (
  ...
  last_seen timestamptz NOT NULL,
  PRIMARY KEY (device_id, kind, namespace, value)
);
...
CREATE TABLE IF NOT EXISTS trust_records (
  ...
  public_key text,
  revision bigint NOT NULL DEFAULT 1,
  owner_established_at timestamptz NOT NULL,
  revoked_at timestamptz
);
CREATE TABLE IF NOT EXISTS access_requests (
  ...
  UNIQUE(device_id, reason_code, state)
);
...
CREATE TABLE IF NOT EXISTS service_entities (
  ...
  associated_device text,
  UNIQUE(provider, cluster_namespace, external_id)
);
CREATE TABLE IF NOT EXISTS collection_leases (
  ...
  PRIMARY KEY(provider, cluster_namespace)
);
...
CREATE TABLE IF NOT EXISTS releases (
  ...
  immutable_blob_path text NOT NULL,
...
CREATE TABLE IF NOT EXISTS audit_events (
  ...
  redacted_outcome jsonb NOT NULL,
