...
export function UpdatesView() {
  ...
  const assignmentByDevice = useMemo(() => new Map(assignments.map((assignment) => [assignment.deviceId, assignment])), [assignments]);
  ...
  return (
    <section className="workspace-grid updates-view">
      <div className="section-heading">
        <div>
          <Badge variant="outline"><ShieldCheck size={13} />Signed lifecycle</Badge>
          <h2>Updates, on your terms</h2>
          <p>Only releases that pass manifest, publisher trust, platform, digest, and compatibility checks become assignable.</p>
        </div>
        <Button variant="ghost" size="icon" aria-label="Refresh update state" onClick={() => refresh().catch(() => setError("Could not refresh update state"))}>
          <RefreshCw size={16} />
        </Button>
      </div>
      ...
      <div className="access-columns">
        <form className="form-panel" onSubmit={importRelease}>
          <div className="panel-title"><Upload size={17} /><h3>Import signed bundle</h3></div>
          <p className="form-help">The signing private key stays outside Scout. Altered, unsigned, oversized, or incompatible bundles are rejected.</p>
          <label>
            Bundle JSON
            <input name="releaseBundle" type="file" accept=".json,application/json" />
          </label>
          <Button type="submit" disabled={busy}><FileKey2 size={15} />Import and verify</Button>
        </form>
        <form className="form-panel" onSubmit={createRollout}>
          <div className="panel-title"><Download size={17} /><h3>Create bounded rollout</h3></div>
          <label>
            Release
            <select required value={selectedRelease} onChange={(event) => setSelectedRelease(event.target.value)}>
              ...
              {releases.filter((release) => !release.revokedAt).map((release) => (
                <option key={release.id} value={release.id}>{displayVersion(release)} · {release.platform}/{release.architecture}</option>
              ))}
            </select>
          </label>
          <label>
          ...
          <label>Device IDs <span className="label-hint">optional; empty means all compatible devices</span><Input value={targets} onChange={(event) => setTargets(event.target.value)} placeholder="device-id-1, device-id-2" /></label>
          <div className="form-inline">
            <label>Concurrency<Input inputMode="numeric" min="1" max="100" value={concurrency} onChange={(event) => setConcurrency(event.target.value)} /></label>
            <label>Canaries<Input inputMode="numeric" min="0" value={canaries} onChange={(event) => setCanaries(event.target.value)} /></label>
            <label>Failure pause<Input inputMode="numeric" min="1" max="100" value={failureThreshold} onChange={(event) => setFailureThreshold(event.target.value)} /></label>
          </div>
          <Button type="submit" disabled={busy || !releases.length}><Download size={15} />Plan rollout</Button>
        </form>
      </div>
      <div className="data-panel">
        <div className="section-heading">
          <div><h3>Release catalog</h3><p>Accepted releases remain immutable. Revocation is visible and prevents new downloads.</p></div>
          ...
          <div className="release-list">
            {releases.map((release) => (
              <div key={release.id}>
                ...
                <span>{release.platform} / {release.architecture} · generation {release.generation} · {release.bytes} bytes</span>
                <Badge variant="outline" className={release.revokedAt ? "access-label" : "enabled-label"}>{release.revokedAt ? "Revoked" : "Verified on import"}</Badge>
                {!release.revokedAt && <Button size="sm" variant="outline" disabled={busy} onClick={() => revokeRelease(release)}>Revoke</Button>}
              </div>
            ))}
          </div>
        ) : (
          <div className="update-features">
            {[["Manual or automatic", "Select desired state; reconnecting agents revalidate assignments."], ["Offline-ready delivery", "Import a signed bundle into an isolated server."], ["Bounded rollback", "A previously verified local slot remains the only automatic rollback target."]].map(([title, description]) => (
              <div key={title}><Check size={17} /><h3>{title}</h3><p>{description}</p></div>
            ))}
          </div>
        )}
      </div>
      <div className="data-panel">
        <div className="section-heading">
          <div><h3>Rollouts</h3><p>Assignments are bounded by release generation and expiry; pause is a safe control boundary.</p></div>
          ...
          <div className="compact-list">
            {rollouts.map((rollout) => (
              <div key={rollout.id}>
                ...
                <span>{rollout.mode} · {rollout.concurrency} concurrent · {rollout.canaries} canaries</span>
                <small>{rollout.paused ? "Paused" : "Active"} · {rollout.failureCount} failures · revision {rollout.revision}</small>
                {!rollout.paused && <Button size="sm" variant="outline" disabled={busy} onClick={() => pauseRollout(rollout)}><Pause size={13} />Pause</Button>}
              </div>
            ))}
          </div>
      ...
      <div className="data-panel">
        <div className="section-heading">
          <div><h3>Desired versus installed</h3><p>Installed version is reported by the device; desired version comes from its current assignment.</p></div>
          <Badge variant="outline">{devices.length}</Badge>
        </div>
        {devices.length ? (
          <div className="table-scroll">
            <table>
              <thead><tr><th>Device</th><th>Installed</th><th>Desired</th><th>Assignment</th></tr></thead>
              <tbody>
                {devices.map((device) => {
                  ...
                  return (
                    <tr key={device.id}>
                      <td><strong>{device.displayName}</strong><small>{device.id}</small></td>
