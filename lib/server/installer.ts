...
export function renderInstallerScript(input: {
  ...
  const lines = [
    ...
    "lock_dir=/var/run/scout-agent-install.lock",
    "lock_acquired=0",
    "cleanup() {",
    ...
    '  run_privileged rm -rf "$lock_dir" 2>/dev/null || true',
    "}",
    "",
    "acquire_install_lock() {",
    "  run_privileged install -d -m 0755 /var/run",
    "  run_privileged sh -c '",
    "    lock=$1; pid=$2",
    ...
    '    run_privileged install -d -m 0700 "$SCOUT_AGENT_ROOT"',
    '    run_privileged install -d -m 0755 "$SCOUT_AGENT_ROOT/releases" "$release_dir" /usr/local/lib/scout-agent',
