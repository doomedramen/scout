...
export function retryEnrollment(systemId: string, now = Date.now()): RetryReceipt {
  ...
  const retry = sqlite.transaction(() => {
    const system = sqlite
      ...
      .get(systemId) as { id: string; excluded: number } | undefined;
    ...
    if (current) {
      ...
      sqlite
        ...
        .run(jobId, systemId, credential.id, now, now);
    }
    sqlite
...
export function decommissionSystem(
  ...
  const decommission = sqlite.transaction(() => {
    const system = sqlite.prepare("SELECT id FROM system WHERE id = ? LIMIT 1").get(systemId);
    ...
    sqlite
      ...
      .run(randomUUID(), systemId, JSON.stringify({ uninstall: "pending/manual" }), now);
    return { systemId, status: "revoked" as const, uninstall: "pending/manual" as const };
  });
  return decommission();
}
