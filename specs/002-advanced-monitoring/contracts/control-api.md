...
**DestinationInput**: name, baseUrl, topic, optional token, enabled=false by default, and optional `allowPlainHttp` (default false). Topic/token are write-only; omitted PATCH secrets retain current values, explicit null token removes authentication, topic cannot be null. Response: id/name/baseUrl/maskedTopic/hasToken/allowPlainHttp/enabled/revision/lastTestAt. Server normalizes topic as a single path segment and disallows reserved ntfy routing names; max 128 ASCII letters/digits/underscore/hyphen. Token max 4096 bytes; name max 120. Plain HTTP is accepted only for private or loopback endpoints when this explicit opt-in is set. Rules default to all enabled destinations; per-provider routing/escalation is outside 002.
