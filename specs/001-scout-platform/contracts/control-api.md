...
| GET/POST /credentials           | metadata on reads; kind, authMethod, username, secret, allowedUse/targets on create | Metadata only after creation. Secret is write-only and never echoed. SSH supports `password` or `private_key` authentication; the username and method are stored as non-secret metadata.                                  |
